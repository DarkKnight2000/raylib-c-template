...
PROJECT_OUTPUT_PATH ?= $(PROJECT_ROOT_PATH)\build\$(PROJECT_VERSION)\$(PLATFORM_NICKNAME)
...
INCLUDE_PATHS = -I$(PROJECT_ROOT_PATH)/include/ -I$(RAYLIB_PATH)/src -I$(RAYLIB_PATH)/src/external
...
ifeq ($(PLATFORM),PLATFORM_ANDROID)
    MAKEFILE_PARAMS = -f Makefile.Android PROJECT_BUILD_PATH=$(PROJECT_OUTPUT_PATH)

    # For Linux and macOS set make and makefile
    # ifeq ($(PLATFORM_OS),LINUX)
    #     MAKEFILE_PARAMS = -f makefiles/Makefile.Android.linux
    # endif
    # ifeq ($(PLATFORM_OS),OSX)
    #     MAKEFILE_PARAMS = -f makefiles/Makefile.Android.macos
    # endif
    export PROJECT_NAME
...
$(PROJECT_NAME): $(OBJS)
	...
	$(CC) -o $(PROJECT_OUTPUT_PATH)/$(PROJECT_NAME)$(EXT) $(OBJS) $(CFLAGS) $(INCLUDE_PATHS) $(LDFLAGS) $(LDLIBS) -D$(PLATFORM)
